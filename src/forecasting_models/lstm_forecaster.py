...
class LSTMForecaster(TorchForecaster):
    def __init__(self, training_parameters, forecasting_model_data):
        super().__init__(training_parameters, forecasting_model_data)
        self._output_size = 1
        ...
        self._lstm = nn.LSTM(self._input_size, self._hidden_size, self._num_layers, batch_first=True)
    ...
    def _forward_pass(self, x):
        ...
        output, (h_n, c_n) = self._lstm(x, (h_0.detach(), c_0.detach()))
        output = output[:, -1, :]
    ...
    def _model_save_name(self):
        return f"lstm_model_{int(time.time())}"
